#!/usr/bin/env bash
ansible_workdir=/tmp/ansible-setup
...
# Clone git repo
su - ${ssh_user} -c "mkdir -p $ansible_workdir"
su - ${ssh_user} -c "cd $ansible_workdir ; git clone -b sid ${git_repo} ."

# Populate Ansible inventory file
cat > $ansible_workdir/hosts <<EOF
[vault]
%{ for ip in vault_ips ~}
...
cat >> $ansible_workdir/vars.yml <<EOF
...
su - ${ssh_user} -c "cd $ansible_workdir ; ansible-playbook site.yml"
